...
def fetch_usgs_chunk(starttime, endtime, minmagnitude=6.0, limit=20000):
    """
    Descarga un bloque de eventos desde USGS.
    """
    params = {
        "format": "geojson",
        "starttime": starttime,
    ...
    try:
        response = requests.get(USGS_EVENT_URL, params=params, timeout=180)
    except Exception as e:
        print("Error de red consultando USGS:", e)
        return []
    ...
    try:
        ...
        print("Error parseando respuesta USGS:", e)
        return []
...
def fetch_usgs_range(start, end, minmagnitude=6.0, chunk_days=365):
    ...
    while current <= end_dt:
        ...
        print(
            f"Descargando USGS: {current.strftime('%Y-%m-%d')} -> "
        ...
        if len(features) >= 19999:
            print(
                "Advertencia: el bloque alcanzó el límite de USGS. "
...
def parse_usgs_features(features):
    """
    Convierte features GeoJSON de USGS en DataFrame.
    """
    rows = []
    ...
    for feature in features:
        try:
            ...
            print("Error parseando feature USGS:", e)
...
def add_country(events):
    """
    Agrega país usando el campo place de USGS.
    """
    if events.empty:
        return events
...
def deduplicate_events(events, time_tol_min=10.0, dist_km=120.0, mag_tol=0.7):
    ...
    Conserva preferentemente USGS sobre semilla manual.
    """
    if events.empty:
        return events
...
def run_pipeline(
    ...
    print(f"Eventos descargados desde USGS: {len(events)}")
...
def generate_alerts(events, forecasts, now=None):
    ...
    if events is not None and not events.empty:
        ...
        if "phenomenon_type" in ev.columns:
            db = rec[ev["phenomenon_type"] == "Doblete"]
...
def _extract_usgs_series(data, element):
    """
    Extrae serie de valores desde estructura USGS geomag.
...
def fetch_multisensor(lat, lon, events=None):
    ...
    """
    sensors = []
    note = ("Fuentes: USGS geomagnetismo (magnetómetro y campo eléctrico), "
    ...
    # ========================================================
    # 1) CAMPO MAGNÉTICO (H) desde observatorio USGS cercano
    # ========================================================
    if obs:
        code, name, dist = obs
        ...
        if len(series) > 3:
            ...
            sensors.append({
                ...
                "status": st,
                "color": col,
                "source": f"USGS {code} ({int(dist)} km)"
            })
        else:
            sensors.append({
                "name": f"Campo magnético (H)",
...
def build_subsurface(events, lat=None, lon=None):
    out = {"cross_section": [], "moho_km": None,
           "note": "Corte construido con hipocentros reales (USGS). Moho según CRUST1.0."}
    if events is not None and not events.empty:
        ev = events.copy()
...
def _embedded_fallas():
    ...
    for i, (nombre, coords, tipo, act) in enumerate(FALLAS_GEOLOGICAS):
        ...
        long_km = _line_length_km(lonlat)
        out.append({
            "id": f"EMB-{i}", "nombre": nombre, "nombre_alternativo": "",
            "pais": "—", "continente": "—", "sistema_falla": "—",
            "tipo_falla": tipo, "actividad": act, "nivel_confianza": "alto",
            "lat_inicio": coords[0][0], "lon_inicio": coords[0][1],
            "lat_fin": coords[-1][0], "lon_fin": coords[-1][1],
            "longitud_km": long_km,
            "slip_rate_min": None, "slip_rate_max": None,
            "mw_max_estimado": _mw_from_length(long_km),
            "profundidad_min": None, "profundidad_max": None,
            "recurrencia_min": None, "recurrencia_max": None,
            "coords": coords, "fuente": "Catálogo embebido",
            "referencia": "", "fecha_actualizacion": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
        })
    return out
...
def fetch_all_faults(force=False):
    ...
    # 2) USGS Quaternary Fault & Fold Database
    try:
