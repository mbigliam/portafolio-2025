...
# Feeds en tiempo real de USGS (los más rápidos del mundo)[cite: 1, 2]
RT_FEEDS = {
...
# ============================================================
# SISMOS EN TIEMPO REAL (antes que nadie)[cite: 1, 2]
# ============================================================
...
def fetch_realtime_quakes():
    ...
    for key, url in RT_FEEDS.items():
        ...
        if isinstance(data, dict):
            df = core.parse_usgs_features(data.get("features", []))
...
# ============================================================
# Campo magnético + clima espacial REAL[cite: 1, 2]
# ============================================================
...
def fetch_spaceweather_extra():
    """Datos reales adicionales de NOAA SWPC[cite: 1, 2]."""
...
def build_cache(force=False):
    with _lock:
        ...
        try:
            print(f"Descargando USGS {start} -> {end}[cite: 1, 2]")
...
def rt_updater():
    """Hilo que actualiza sismos en tiempo real cada 60 s[cite: 1, 2]."""
    while True:
...
L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
  ...
  className: 'map-tiles-dark' // Opcional para oscurecerlo si prefieres estilo oscuro
}).addTo(map);
...
/* ----------------------------------------------------------
   ...
   ---------------------------------------------------------- */
/* ----------------------------------------------------------
   5b) FALLAS: línea azul segmentada + nombre
   ---------------------------------------------------------- */
/* ----------------------------------------------------------
   5b) FALLAS: línea azul segmentada + nombre
   ---------------------------------------------------------- */
function renderFallas(data) {
  try {
    (data.fallas || []).forEach(function (f) {
      ...
      // Línea azul segmentada bien visible para las fallas
      ...
      // Etiqueta de la falla
      var mid = coords[Math.floor(coords.length / 2)];
      L.marker(mid, {
        icon: L.divIcon({
          className: "falla-label",
          html: '<div class="falla-nombre" style="color: #3aa0ff; font-weight: bold; background: rgba(11,18,32,0.8); padding: 2px 4px; border-radius: 4px;">' + (f.nombre || "") + '</div>',
          iconSize: [130, 14], iconAnchor: [65, 7]
        })
      }).addTo(G.falla_labels);
    });

    // Control de visibilidad de etiquetas según el zoom del mapa
    function toggleLabels() {
      if (map.getZoom() >= 5) { 
        if (!map.hasLayer(G.falla_labels)) G.falla_labels.addTo(map); 
      } else { 
        map.removeLayer(G.falla_labels); 
      }
    }
...
function renderTable(data) {
  ...
  var live = (data.rt_events || []).slice(0, 30).map(function (e) {
    var d = new Date(e.time_utc || e.time);
    var fechaStr = !isNaN(d.getTime()) ? d.toLocaleDateString() : "-";
    var horaStr = !isNaN(d.getTime()) ? d.toLocaleTimeString() : "-";
    var anioVal = !isNaN(d.getTime()) ? d.getUTCFullYear() : "-";
    return "<tr><td>" + (e.country || "Chile/Global") + "</td><td>" + (e.place || "") + '</td><td><span class="badge red">🔴 EN VIVO</span></td><td>' +
        anioVal + "</td><td>" + fechaStr + "</td><td>" + horaStr +
...
setInterval(load, 60000);       // sismos/fenómenos cada 60 s (antes que nadie)[cite: 1, 2]
